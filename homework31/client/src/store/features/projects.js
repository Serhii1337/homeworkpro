...
export const getProjectsAsync = createAsyncThunk('projects/getList', async () => {
  const result = await axios.get(PROJECTS_URL, { headers: authHeaders() });
  return result.data;
});

export const saveProjectAsync = createAsyncThunk('projects/save', async project => {
  const result = await axios.post(PROJECTS_URL, project, { headers: authHeaders() });
  return result.data;
})

export const deleteProjectAsync = createAsyncThunk('projects/delete', async (id) => {
  const result = await axios.delete(`${PROJECTS_URL}/${id}`, { headers: authHeaders() });
  return id;
});
